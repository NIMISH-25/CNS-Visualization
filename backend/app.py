...
def execute_python_code(code):
    ...
    try:
        ...
        def new_plot(fig, filename, *args, **kwargs):
            ...
            return orig_plot(
                fig, filename=filename_new, auto_open=False, *args, **kwargs
            )
    ...
    if "__final_fname__" not in safe_globals:
        return None, (
            "Visualization not generated. Please ensure your code includes "
...
def execute_r_code(code):
    ...
    modified_code = re.sub(
        r'filename\s*=\s*["\'].*?["\']', "filename=output_file", code
    )
    modified_code = re.sub(
        r'file\s*=\s*["\'].*?["\']', "file=output_file", modified_code
    )

    code_with_filepath = (
        f"output_file <- '{filepath.replace(os.sep, '/')}'\n" + modified_code + "\n"
    )
    ...
    try:
        result = subprocess.run(
            ["Rscript", temp_r_file], capture_output=True, text=True, timeout=30
        )
    ...
    if not os.path.exists(filepath):
        return None, (
            "Visualization not generated. Please ensure your R code writes the image or widget "
...
def visualize():
    ...
    if language == "python":
        ...
        return (
            jsonify({"error": "Unsupported language. Please choose Python or R."}),
            400,
        )
